...
{
  this.options = options || { ayBuffer: true, dictFormat: false };
  this.buffer = buffer;
...
DBusBuffer.prototype.readTree = function readTree(tree) {
  ...
  case 'a':
    ...
    var res= this.readArray(tree.child[0], arrayBlobLength);
    return res;
    case 'v':
...
DBusBuffer.prototype.readArray = function readArray(eleType, arrayBlobSize) {
  ...
  */
  ...
  var result = [];
  if(this.options.dictFormat===true) {
    var dict={};
    while(this.pos < end) {
      var tree=this.readTree(eleType);
      result.push(tree);
      //if( tree.length === 2 &&
          // !(tree[0] instanceof Array) &&
          // (tree[0].type === 'a') &&
          // (tree[0].child instanceof Array) &&
          // (tree[0].child.length === 2) &&
          // (tree[0].child[0].type === '{') &&
          // ('ybnquigsoxtd'.indexOf(tree[0].child[0].child) > -1) && //the first single complete type (the "key") must be a basic type
          // ('ybnquigsoxtdv'.indexOf(tree[0].child[1].child) > -1)) {

      if(tree.length === 2 &&
          !(tree[0] instanceof Array) &&
          (tree[1] instanceof Array) &&
          (tree[1].length === 2) &&
          (tree[1][0] instanceof Array) &&
          (tree[1][0].length === 1) &&
          !(tree[1][0][0] instanceof Array) &&
          // (tree[1][0][0]['type'] !== undefined) &&
          (tree[1][0][0]['type'].length === 1)) {
        dict[tree[0]]=tree[1][1][0];
      }
    }
    if(Object.keys(dict).length >=result.length)
      return dict;
    else return result;
  } else {
    while(this.pos < end) {
      var res=this.readTree(eleType);
      result.push(res);
    }
    return result;
  }
