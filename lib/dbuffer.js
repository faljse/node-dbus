...
{
  console.log('init len' + buffer.length);
...
DBusBuffer.prototype.readVariant = function readVariant() {
  ...
  console.log("readVariant: " + signature +" "+this.pos);
  if(signature.length==1 && 'ybnquigsoxtd'.indexOf(signature)>-1)
  {
    console.log("ret:...");
    var ret=this.readSimpleType(signature);
    console.log("ret simple:"+ret);
    return ret;
  }
...
DBusBuffer.prototype.readArray = function readArray(eleType, arrayBlobSize) {
  ...
  if (isHash(eleType)) {
    ...
    while(this.pos < start + arrayBlobSize) {
      console.log("read var:"+this.pos);
      this.align(3);
      key = this.readSimpleType('s');
      var res=result[key] = this.readVariant();
...
DBusBuffer.prototype.readSimpleType = function readSimpleType(t) {
  ...
  case 'o':
    ...
    console.log("read string "+t +" len "+len)
    return this.readString(len);
